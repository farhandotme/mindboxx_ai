...
# Premium monospace stack — tries nicer coding fonts first, falls back
# gracefully to whatever the OS actually has installed.
_MONO_FAMILIES = ["Cascadia Code", "JetBrains Mono", "Consolas",
...
class HudCanvas(QWidget):
    # Set True to render a loaded face_path image instead of the drawn
    ...
    def __init__(self, face_path: str, assistant_name: str = "MINDBOXX", parent=None):
        ...
        # in a slowly-rotating circle around the HUD, connected by dotted
        # lines like a constellation. Fed by set_plugins() from MainWindow.
    ...
    def set_plugins(self, badges: list[dict]) -> None:
        """Feed the orbiting ring its badge list. Each badge is
        {"name": str, "icon": str, "enabled": bool}. Safe to call repeatedly
        (e.g. from a polling timer) — only triggers a repaint if changed."""
        if badges != self._ring_badges:
            self._ring_badges = list(badges)
    ...
    def _make_stars(self, W: int, H: int) -> QPixmap:
        """Pre-render a starfield + soft planetary-limb glow so the HUD reads
        ...
        rnd = random.Random(1337)   # fixed seed → stable field across resizes
        for _ in range(230):
            x = rnd.uniform(0, W)
            y = rnd.uniform(0, H * 0.85)
            r = rnd.choice([0.5, 0.6, 0.8, 0.8, 1.1, 1.4])
            a = rnd.randint(35, 190)
            gp.setBrush(QBrush(qcol(C.WHITE, a)))
            gp.drawEllipse(QPointF(x, y), r, r)

        for _ in range(14):   # a few brighter stars with a tiny bloom
            x = rnd.uniform(0, W)
            y = rnd.uniform(0, H * 0.85)
            glow = QRadialGradient(QPointF(x, y), 3.2)
            glow.setColorAt(0.0, qcol(C.WHITE, 215))
    ...
    def _step(self):
        ...
        # Integration ring drifts slowly — a calm orbit, not a spinner.
        self._ring_angle = (self._ring_angle + 0.12) % 360
    ...
    def paintEvent(self, _):
        ...
        # only on resize.
        _skey = (W, H)
        if self._stars_cache is None or self._stars_key != _skey:
            self._stars_cache = self._make_stars(W, H)
            self._stars_key   = _skey
        p.drawPixmap(0, 0, self._stars_cache)
        ...
        # No in-canvas status text/waveform — the AUREX look keeps the ring
        # clean; state + waveform now live in the bottom bar (see MainWindow
        # ._build_footer / ._apply_state).
        p.end()   # end deterministically so the backing store never flushes an active painter

    def _draw_ring(self, p: QPainter, cx: float, cy: float, fw: float) -> None:
        """Draws the orbiting badge ring: one small glass circle per live
        integration (voice, wake word, memory, camera, clipboard, plugins),
        sitting on a faint dotted orbit path that slowly drifts around the
        HUD. Kept well inside the tick-mark band so it never collides with
        the frame or corner brackets."""
        badges = self._ring_badges
        n = len(badges)
        if n == 0:
            return

        badge_r = max(9, min(14, fw * 0.026))
        ring_r  = fw * 0.40
        ring_r  = min(ring_r, min(cx, cy) - badge_r - 4)
        if ring_r <= 0:
            return

        positions = []
        for i in range(n):
            ang = math.radians(self._ring_angle + i * (360.0 / n) - 90.0)
            positions.append((cx + ring_r * math.cos(ang), cy + ring_r * math.sin(ang)))

        # faint dotted orbit path — a single continuous ring, not per-badge
        # chords, so it reads as one calm path rather than a jagged polygon.
        p.setPen(QPen(qcol(C.PRI_DIM, 55), 1, Qt.PenStyle.DotLine))
        p.setBrush(Qt.BrushStyle.NoBrush)
        p.drawEllipse(QPointF(cx, cy), ring_r, ring_r)

        # badges themselves — glassy circle + icon glyph
        for badge, (x, y) in zip(badges, positions):
            enabled = badge.get("enabled", True)
            col = C.PRI if enabled else C.TEXT_DIM
            glow_a = 130 if enabled else 50

            for gi in range(2, 0, -1):
                gr  = badge_r + gi * 2.0
                a   = int(glow_a * (0.35 / gi))
                p.setPen(QPen(qcol(col, a), 1))
                p.setBrush(Qt.BrushStyle.NoBrush)
                p.drawEllipse(QPointF(x, y), gr, gr)

            p.setPen(Qt.PenStyle.NoPen)
            p.setBrush(QBrush(qcol(C.BG, 240)))
            p.drawEllipse(QPointF(x, y), badge_r, badge_r)
            p.setPen(QPen(qcol(col, 220), 1.2))
            p.setBrush(Qt.BrushStyle.NoBrush)
            p.drawEllipse(QPointF(x, y), badge_r, badge_r)

            p.setPen(QPen(qcol(col, 240)))
            p.setFont(QFont("Segoe UI Symbol", max(8, int(badge_r * 0.85)), QFont.Weight.Bold))
            p.drawText(QRectF(x - badge_r, y - badge_r, badge_r * 2, badge_r * 2),
...
class MetricBar(QWidget):
    ...
    text) call signature so every existing caller keeps working unchanged."""

    def __init__(self, label: str, color: str = C.PRI, icon: str = "●",
                 status: str = "Online", parent=None):
        super().__init__(parent)
        self._label  = label
        self._color  = color
        self._icon   = icon
    ...
    def paintEvent(self, _):
        ...
        # icon badge
        icon_r = 13.0
        icx, icy = 10 + icon_r, H / 2 - 8
        p.setPen(QPen(qcol(self._color, 210), 1.3))
        p.setBrush(QBrush(qcol(C.BG, 220)))
        p.drawEllipse(QPointF(icx, icy), icon_r, icon_r)
        p.setPen(QPen(qcol(self._color, 235)))
        p.setFont(QFont("Segoe UI Symbol", 10))
        p.drawText(QRectF(icx - icon_r, icy - icon_r, icon_r * 2, icon_r * 2),
        ...
        p.drawText(QRectF(text_x, 6, W - text_x - 18, 14),
                   Qt.AlignmentFlag.AlignLeft | Qt.AlignmentFlag.AlignVCenter, self._label)
        p.setPen(Qt.PenStyle.NoPen)
        p.setBrush(QBrush(qcol(self._color, 230)))
        p.drawEllipse(QPointF(W - 12, 13), 3, 3)

        # status word
        p.setFont(mono_font(7))
        p.setPen(QPen(qcol(C.TEXT_DIM), 1))
        p.drawText(QRectF(text_x, 20, W - text_x - 14, 12),
                   Qt.AlignmentFlag.AlignLeft | Qt.AlignmentFlag.AlignVCenter, self._status)

        # thin progress bar + trailing percentage
        bar_h, bar_y = 3, H - 14
        bar_x, bar_w = text_x, W - text_x - 36
        p.setPen(Qt.PenStyle.NoPen)
        ...
        p.drawText(QRectF(W - 34, H - 20, 28, 14),
...
class LogWidget(QTextEdit):
    ...
    def append_stream(self, title: str, description: str) -> None:
        """AUREX-style Intelligence Stream entry — a bullet, a bold caps
        title with a timestamp, and a dim description line underneath
        (NEURAL SYNC / CONTEXT READY / etc. in the reference). Internally
        this is just append_log() with a reserved separator, so it shares
        the same queue/typewriter machinery as plain log lines."""
        self.append_log(f"{title} || {description}")
    ...
    def _next(self):
        ...
        # "TITLE || description" — a structured stream entry, rendered as a
        # bullet + bold caps title + timestamp, then a dim description line.
        # Anything without " || " (every existing call site) keeps behaving
        # exactly as a plain single-line log entry.
        if " || " in raw:
            title, _, desc = raw.partition(" || ")
            ts = time.strftime("%H:%M")
            header = f"\u25cf  {title.strip().upper()}   {ts}"
            self._text      = f"{header}\n      {desc.strip()}"
    ...
    def _step(self):
        if self._pos < len(self._text):
            ...
            fmt = cur.charFormat()
            if self._tag == "stream":
                in_header = self._pos <= (self._hdr_len or 0)
                f = fmt.font()
                f.setBold(in_header)
                fmt.setFont(f)
                col = qcol(C.GREEN) if ch == "\u25cf" else (
                    qcol(C.PRI) if in_header else qcol(C.TEXT_DIM))
            else:
                col = {
                    "you":  qcol(C.WHITE),
                    "ai":   qcol(C.PRI),
...
class _DropCanvas(QWidget):
    ...
    def paintEvent(self, _):
        ...
        grad.setColorAt(0.0, qcol(bg_top))
        grad.setColorAt(1.0, qcol(bg_bot))
        p.setBrush(QBrush(grad)); p.setPen(Qt.PenStyle.NoPen)
        p.drawRoundedRect(rect, 10, 10)

        if z._current_file:   border_col = qcol(C.GREEN, 200)
        elif z._drag_over:    border_col = qcol(C.PRI, 230)
        elif z._hovering:     border_col = qcol(C.BORDER_B, 210)
...
class _CameraPreview(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        title.setFont(mono_font(7, QFont.Weight.Bold))
        title.setStyleSheet(f"color: {C.PRI}; background: transparent;")
        hdr.addWidget(title)
        hdr.addStretch()
        close_btn = QPushButton("✕")
        close_btn.setFixedSize(16, 16)
        close_btn.setFont(mono_font(8))
...
class SetupOverlay(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        for key, label in [("windows","⊞  Windows"),("mac","  macOS"),("linux","🐧  Linux")]:
            ...
            btn.setFont(mono_font(9, QFont.Weight.Bold))
        ...
        init_btn.setFont(mono_font(10, QFont.Weight.Bold))
...
class CustomizeOverlay(QWidget):
    ...
    def __init__(self, assistant_name="MINDBOXX", user_name="",
        ...
        self._name_input.setFont(mono_font(10))
        ...
        self._user_input.setFont(mono_font(10))
        ...
        for _v in AVAILABLE_VOICES:
            ...
            b.setCheckable(True)
            b.setFixedHeight(28)
            b.setFont(mono_font(8, QFont.Weight.Bold))
        ...
        df_btn.setFont(mono_font(7, QFont.Weight.Bold))
        ...
        save_btn.setFont(mono_font(9, QFont.Weight.Bold))
        ...
        cancel_btn.setFont(mono_font(9))
...
class PluginManagerOverlay(QWidget):
    ...
    def _build_row(self, p: dict) -> QHBoxLayout:
        ...
        btn.setFont(mono_font(7, QFont.Weight.Bold))
        if not p["valid"]:
            btn.setText("BROKEN")
...
class ConfirmBanner(_HudOverlay):
    ...
    def __init__(self, title: str, detail: str, parent=None):
        ...
        hdr = QLabel("⚠  CONFIRM")
        hdr.setFont(mono_font(11, QFont.Weight.Bold))
        hdr.setStyleSheet(f"color: {C.ACC}; background: transparent;")
        lay.addWidget(hdr)

        ttl = QLabel(title)
        ttl.setWordWrap(True)
        ttl.setFont(mono_font(10, QFont.Weight.Bold))
        ...
        yes.setFont(mono_font(9, QFont.Weight.Bold))
...
class AudioDeviceOverlay(_HudOverlay):
    """Choose which microphone AUREX listens to and which speakers it uses.
    ...
    is plugged in. 'AUREX can't hear me' is usually 'AUREX is listening to the
    webcam'."""
    ...
    def __init__(self, parent=None):
        ...
        self._in_box  = _row("MICROPHONE — what AUREX hears you with",
                             "input", get_input_device())
        lay.addSpacing(4)
        self._out_box = _row("SPEAKERS — what AUREX talks through",
        ...
        ok.setFont(mono_font(9, QFont.Weight.Bold))
...
class MemoryOverlay(_HudOverlay):
    """Everything AUREX has stored about you, and when it learned it.
    ...
    def _rebuild(self):
        ...
        cap.setFont(mono_font(7))
        cap.setStyleSheet(f"color: {C.TEXT_DIM}; background: transparent;")
        self._lay.addWidget(cap)

        if not rows:
            empty = QLabel("Nothing stored yet.")
            empty.setFont(mono_font(9))
...
class ClipboardPanel(QWidget):
    """Floating panel shown when text is copied — offers quick AUREX actions."""
    ...
    def __init__(self, parent=None):
        ...
        icon_lbl.setFont(mono_font(7, QFont.Weight.Bold))
        icon_lbl.setStyleSheet(f"color: {C.ACC2}; background: transparent;")
        hdr.addWidget(icon_lbl); hdr.addStretch()
        x_btn = QPushButton("✕")
        x_btn.setFixedSize(16, 16)
        x_btn.setFont(mono_font(8))
        ...
        for label, cmd_fmt in [
            ...
            b = QPushButton(label)
            b.setFixedHeight(22)
            b.setFont(mono_font(7, QFont.Weight.Bold))
            b.setCursor(Qt.CursorShape.PointingHandCursor)
            b.setStyleSheet(_bs)
    ...
    def _build_section(self, form: QVBoxLayout, sec: dict):
        ...
        for field in fields:
            ...
            if ftype == "choice":
                ...
                w.setFixedHeight(28)
                w.setFont(mono_font(8, QFont.Weight.Bold))
        ...
        if isinstance(action, dict) and callable(action.get("run")):
            ...
            ab.setFont(mono_font(8, QFont.Weight.Bold))
...
class RemoteKeyOverlay(QWidget):
    ...
    def __init__(self, url: str, key: str, auto_login_url: str = "",
        ...
        self._key_lbl.setFont(mono_font(28, QFont.Weight.Bold))
        ...
        self._timer_lbl.setFont(mono_font(8))
        ...
        new_btn.setFont(mono_font(8, QFont.Weight.Bold))
    ...
    def _update_qr(self, url: str) -> None:
        ...
        try:
            ...
            self._qr_label.setFont(mono_font(8))
            self._qr_label.setStyleSheet(
    ...
    def mark_connected(self) -> None:
        ...
        """)
        self._qr_label.setText("✓")
        self._qr_label.setFont(mono_font(54, QFont.Weight.Bold))
        self._qr_label.setStyleSheet(
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, face_path: str):
        ...
        self.get_plugins       = None   # callable: () -> list[dict], set by AUREXLive
        self.get_plugin_settings = None # callable: () -> list[dict] settings schemas, set by AUREXLive
        self.on_wake_toggle    = None   # callable: (enable: bool) -> str, set by AUREXLive
        ...
        # Orbiting integration ring — always-on core subsystems plus whatever
        # plugins are currently discovered/enabled. Polled rather than
        # event-driven since plugin state can change from other threads.
        self._ring_timer = QTimer(self)
        self._ring_timer.timeout.connect(self._refresh_ring)
        self._ring_timer.start(3000)
        QTimer.singleShot(200, self._refresh_ring)
        ...
        _cam_title.setFont(mono_font(8, QFont.Weight.Bold))
        _cam_title.setStyleSheet(f"color: {C.PRI}; background: transparent;")
        _cam_hdr.addWidget(_cam_title)
        _cam_hdr.addStretch()
        _cam_x = QPushButton("✕  CLOSE")
        _cam_x.setFont(mono_font(8, QFont.Weight.Bold))
    ...
    # ------------------------------------------------------------------
    @staticmethod
    def _build_AUREX_icon(out_path: Path) -> bool:
        """
        Render a AUREX arc-reactor icon at 4× resolution and downsample
    ...
    def _update_metrics(self):
        ...
        # NET — the card now reads as "SIGNAL" strength rather than raw
        # throughput (matching the AUREX reference's clean percentage look),
        # so show a percentage with a healthy floor instead of dropping to
        # 0% whenever the assistant is simply sitting idle.
        net = snap["net"]
        net_pct = max(72.0, min(100.0, 72.0 + net * 10))
        self._bar_net.set_value(net_pct, f"{net_pct:.0f}%")

        # GPU — doubles as the "LINK" status card. No GPU to read (or one
        # hidden by a VM/sandbox) isn't the same as being disconnected, so
        # show a steady, healthy reading instead of a broken-looking N/A.
        gpu = snap["gpu"]
        if gpu >= 0:
            self._bar_gpu.set_value(gpu, f"{gpu:.0f}%")
        else:
            self._bar_gpu.set_value(100, "100%")
    ...
    def _build_header(self) -> QWidget:
        ...
        left_row.addWidget(self._drawer_btn)
        left_row.addStretch()
        left_col.addLayout(left_row)
        lay.addLayout(left_col)
        lay.addStretch(1)

        # ── center: wordmark, set inside a chamfered hex frame ───────────────
    ...
    def _build_left_panel(self) -> QWidget:
        ...
        for bar in (self._bar_cpu, self._bar_net, self._bar_gpu, self._bar_mem):
            lay.addWidget(bar)
        ...
        self._proc_lbl.setFont(mono_font(8))
        ...
        tagline = QLabel("INTELLIGENCE\nFINDS A WAY")
        tagline.setFont(mono_font(7, QFont.Weight.Bold))
        tagline.setStyleSheet(f"color: {C.TEXT_DIM}; background: transparent; letter-spacing: 1px;")
        lay.addWidget(tagline)

        dash = QLabel("—")
        dash.setStyleSheet(f"color: {C.BORDER_B}; background: transparent;")
        lay.addWidget(dash)
    ...
    def _build_right_panel(self) -> QWidget:
        ...
        def _sec(txt):
            l = QLabel(f"▸ {txt}")
            l.setFont(mono_font(7, QFont.Weight.Bold))
            l.setStyleSheet(
                f"color: {C.TEXT_MED}; background: transparent; "
        ...
        stream_hdr = QHBoxLayout(); stream_hdr.setSpacing(6)
        stream_hdr.addWidget(_sec("INTELLIGENCE STREAM"))
        ...
        # Command-input row now lives in the bottom bar (see _build_footer),
        ...
        quote = QLabel(f'"A more intuitive tomorrow, together."')
        quote.setFont(QFont("Cascadia Code", 7, QFont.Weight.Normal, italic=True))
        quote.setWordWrap(True)
        quote.setAlignment(Qt.AlignmentFlag.AlignCenter)
        quote.setStyleSheet(f"color: {C.TEXT_DIM}; background: transparent; padding-top: 2px;")
        lay.addWidget(quote)
    ...
    def _build_quick_drawer(self) -> QWidget:
        ...
        hdr = QLabel("◈ CONTROLS")
        hdr.setFont(mono_font(7, QFont.Weight.Bold))
        ...
        remote_btn.setFont(mono_font(8, QFont.Weight.Bold))
        ...
        self._autostart_btn.setFont(mono_font(7))
        ...
        self._wake_btn.setFont(mono_font(7))
        ...
        self._wake_sleep_btn.setFont(mono_font(7))
        ...
        plugin_btn.setFont(mono_font(7))
        ...
        settings_btn.setFont(mono_font(7))
    ...
    def _build_content_panel(self) -> QWidget:
        ...
        dot = QLabel("◈")
        dot.setFont(mono_font(9, QFont.Weight.Bold))
        dot.setStyleSheet(f"color: {C.PRI}; background: transparent;")
        hdr.addWidget(dot)

        self._content_title_lbl = QLabel("BRIEFING")
        self._content_title_lbl.setFont(mono_font(8, QFont.Weight.Bold))
        ...
        self._content_display.setFont(mono_font(8))
    ...
    def _build_footer(self) -> QWidget:
        """AUREX-style bottom bar: decorative waveform either side of a big
        ...
        grid_btn.setStyleSheet(f"""
            ...
            QPushButton:hover {{ color: {C.PRI}; border-color: {C.PRI_DIM}; }}
        """)
        grid_btn.clicked.connect(self._drawer_btn.click)
        in_row.addWidget(grid_btn)

        in_row.addLayout(self._build_input_row())
        v.addLayout(in_row)

        self._style_mute_btn()   # sync big mic button + labels to initial mute state
        ...
        def _fl(txt, color=C.TEXT_MED):
            l = QLabel(txt); l.setFont(mono_font(7))
    ...
    def _wake_state(self) -> dict:
        ...
        if self.wake_get_state:
            try:
                ...
                pass
        # Before AUREXLive has wired its callback (drawer built at startup).
    ...
    def _toggle_wake_word(self):
        ...
        if not st["ready"]:
            ...
            return
        # Already downloaded → just flip enabled/disabled through AUREXLive.
    ...
    _CORE_RING_BADGES = [
        {"name": "VOICE",     "icon": "♪", "enabled": True},
        {"name": "WAKE WORD", "icon": "◉", "enabled": True},
        {"name": "MEMORY",    "icon": "▤", "enabled": True},
        {"name": "CAMERA",    "icon": "▣", "enabled": True},
        {"name": "CLIPBOARD", "icon": "❐", "enabled": True},
    ]

    def _refresh_ring(self):
        """Rebuild the HUD's orbiting badge ring from live state: the always
        present core subsystems, plus one badge per discovered plugin."""
        badges = list(self._CORE_RING_BADGES)

        try:
            wake_state = self.wake_get_state() if self.wake_get_state else None
            if wake_state is not None:
                for b in badges:
                    if b["name"] == "WAKE WORD":
                        b["enabled"] = bool(wake_state.get("enabled", True))
        except Exception:
            pass
        ...
        try:
            ...
            for pl in plugins or []:
                if not pl.get("valid", True):
                    continue
                badges.append({
                    "name":    pl.get("name", "PLUGIN"),
                    "icon":    "✦",
    ...
    def _style_mute_btn(self):
        if self._muted:
            ...
            self._mute_btn.setStyleSheet(f"""
                QPushButton {{
                    background: qlineargradient(x1:0, y1:0, x2:0, y2:1,
                        stop:0 #200008, stop:1 #140005);
                    color: {C.MUTED_C};
                    border: 1px solid {C.MUTED_C}; border-radius: 8px;
                    letter-spacing: 1px;
                }}
                QPushButton:hover {{ background: #2a000a; }}
            """)
        else:
        ...
        if not hasattr(self, "_mic_big_btn"):
            return
        if self._muted:
            self._mic_big_btn.setStyleSheet(f"""
                QPushButton {{
    ...
    def _apply_state(self, state: str):
        ...
        if hasattr(self, "_state_lbl"):
            ...
            self._state_sub_lbl.setText(sub_map.get(state.upper(), "Tap and speak"))
    ...
    def _seed_intelligence_stream(self) -> None:
        """Populate the Intelligence Stream with the boot-sequence entries
        shown in the AUREX reference, so a fresh launch reads the same way
        the mockup does instead of starting on a blank panel."""
        for title, desc in (
            ("Neural Sync",           "Cognitive systems aligned"),
            ("Context Ready",         "Environment understood"),
            ("Sensory Field Stable",  "Audio \u2022 Vision \u2022 Text"),
            ("Intent Resolved",       "Standing by for your command"),
        ):
            self._log.append_stream(title, desc)
