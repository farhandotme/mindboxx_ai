...
def _real_profile_dir(browser: str) -> str:
    ...
    fallback = home / ".AUREX_profiles" / browser
...
class _BrowserSession:
    ...
    async def _launch(self):
        ...
        if engine_name == "firefox":
            profile = _firefox_profile_dir() or str(
                Path.home() / ".AUREX_profiles" / "firefox"
            )
            kwargs: dict = {
                "headless":    False,
                "slow_mo":     0,
            ...
            try:
                ...
                print(f"[Browser] Firefox real profile failed ({e}), using AUREX profile")
                AUREX = str(Path.home() / ".AUREX_profiles" / "firefox_AUREX")
                Path(AUREX).mkdir(parents=True, exist_ok=True)
                self._context = await engine_obj.launch_persistent_context(AUREX, **kwargs)
        ...
        if engine_name == "webkit":
            safari_profile = str(Path.home() / ".AUREX_profiles" / "safari")
        ...
        # automation). Fall back to a persistent AUREX automation profile —
        # accounts logged in here once stay logged in on later sessions too.
        AUREX_profile = str(Path.home() / ".AUREX_profiles" / self.browser_name)
        Path(AUREX_profile).mkdir(parents=True, exist_ok=True)
        print(f"[Browser] Retrying with AUREX profile: {AUREX_profile}")

        try:
            self._context = await engine_obj.launch_persistent_context(AUREX_profile, **kwargs)
            self._page = await self._adopt_page()
            print(f"[Browser] ✅ Launched [{label}] with AUREX profile "
    ...
    async def screenshot(self, path: str = None) -> str:
        page = await self._get_page()
        try:
            save_path = path or str(Path.home() / "Desktop" / "AUREX_screenshot.png")
