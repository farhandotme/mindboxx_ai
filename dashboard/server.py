"""
dashboard/server.py — AUREX Local HTTP Dashboard
...
def _make_uploads_dir() -> Path:
    ...
    for candidate in [
        Path.home() / "Downloads" / "AUREX Uploads",
        Path.home() / "Documents" / "AUREX Uploads",
        BASE_DIR / "uploads",
    ]:
        try:
...
# ── AES-256-CBC ───────────────────────────────────────────────────────────────
_AES_SALT = b'AUREX-DASHBOARD-v1'
...
def _ensure_network_access(port: int) -> None:
    ...
    if sys.platform == "win32":
        ...
        port_rule = f"AUREX Dashboard Port {port}"
        prog_rule  = "AUREX Dashboard Python"
        ...
        bat_body = "\r\n".join(bat_lines) + "\r\n"
        fd, bat_path = tempfile.mkstemp(suffix=".bat", prefix="AUREX_fw_")
        try:
        ...
        try:
            ...
            if int(ret) > 32:
                ...
                print("[Dashboard] Phone connections may fail until AUREX is run as Administrator.")
...
class DashboardServer:
    ...
    def _ssl_enabled() -> bool:
        ...
        return (certs / "AUREX.key").exists() and (certs / "AUREX.crt").exists()
...
<p>Press <strong style="color:#dde3ed">Remote Control</strong> in AUREX to get a new QR code.</p>
</div></body></html>""")
...
<body>
<script>
  sessionStorage.setItem('AUREX_token','{tok}');
  sessionStorage.setItem('AUREX_key','{key}');
  localStorage.setItem('AUREX_device_token','{dev_tok}');
  setTimeout(function(){{location.replace('/')}},400);
</script>
<p>Connecting to AUREX…</p>
</body></html>""")
    ...
    async def _serve_alias(self) -> None:
        ...
        ssl_key  = BASE_DIR / "config" / "certs" / "AUREX.key"
        ssl_cert = BASE_DIR / "config" / "certs" / "AUREX.crt"
    ...
    async def serve(self) -> None:
        ...
        print("[Dashboard] Press 'Remote Control' in AUREX UI to get the QR code.")
        await uvicorn.Server(cfg).serve()
